...
function openQuestion(move) {
  ...
  // Agregar botón principal para leer la pregunta en voz alta
  let ttsTitleBtn = document.getElementById('tts-title-btn');
  if (!ttsTitleBtn) {
    ttsTitleBtn = document.createElement('button');
    ttsTitleBtn.id = 'tts-title-btn';
    ttsTitleBtn.innerHTML = '🔊 Escuchar Pregunta';
    ttsTitleBtn.style.marginBottom = '16px';
    ttsTitleBtn.style.background = '#fbbf24';
    ttsTitleBtn.style.border = '3px solid #b45309';
    ttsTitleBtn.style.borderRadius = '12px';
    ttsTitleBtn.style.padding = '8px 16px';
    ttsTitleBtn.style.fontSize = '1.1rem';
    ttsTitleBtn.style.cursor = 'pointer';
    ttsTitleBtn.style.fontFamily = "'Fredoka One', cursive";
    ttsTitleBtn.style.color = '#78350f';
    ttsTitleBtn.style.boxShadow = '0 4px 0 #b45309';
    ttsTitleBtn.style.transition = 'transform 0.1s';
    ttsTitleBtn.onactive = () => { ttsTitleBtn.style.transform = 'translateY(2px)'; };
    
    // Insertar después del título
    const titleEl = document.getElementById('q-title');
    titleEl.parentNode.insertBefore(ttsTitleBtn, titleEl.nextSibling);
  }
  ttsTitleBtn.onclick = () => speakText(randQ.q);
  ...
  shuffledOpts.forEach((optObj) => {
    // Contenedor flexible para alinear el botón de audio con la respuesta
    const optContainer = document.createElement('div');
    optContainer.style.display = 'flex';
    optContainer.style.gap = '8px';
    optContainer.style.width = '100%';

    // Botón de audio para esta opción
    const ttsBtn = document.createElement('button');
    ttsBtn.innerHTML = '🔊';
    ttsBtn.title = 'Escuchar esta opción';
    ttsBtn.style.background = '#93c5fd';
    ttsBtn.style.border = '3px solid #2563eb';
    ttsBtn.style.borderRadius = '12px';
    ttsBtn.style.padding = '0 12px';
    ttsBtn.style.fontSize = '1.2rem';
    ttsBtn.style.cursor = 'pointer';
    ttsBtn.style.boxShadow = '0 4px 0 #2563eb';
    ttsBtn.style.display = 'flex';
    ttsBtn.style.alignItems = 'center';
    ttsBtn.style.justifyContent = 'center';
    ttsBtn.onclick = (e) => {
      e.stopPropagation(); // Evitar seleccionar la respuesta
      speakText(optObj.text);
    };
    ...
    btnBox.appendChild(optContainer);
  });
